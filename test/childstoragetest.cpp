...
ChildStorageTest::ChildStorageTest()
{
  ...
  res = db->open(nullptr, "ChildStorageTest_testStorageCreation.db", "master",
                 DB_BTREE, DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);
  ...
  res = secdb->open(nullptr, "ChildStorageTest_testStorageCreation.db",
                    "secondary", DB_BTREE,
  ...
  res = parent_db->open(nullptr, "ChildStorageTest_testStorageCreation.db",
                        "parent", DB_BTREE,
                        DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);
...
void ChildStorageTest::testStorageCreation()
{
  ...
  using ParentDeleterType =
      ParentsDeleter<decltype(get_id(std::declval<TestElement>())), TestElement,
                     ChildContainerType>;
  using ParentContainerType =
      Storage<TestElement, TestMarshaller, TestWatcher,
              DefaultTransactionManager, ParentDeleterType>;

  std::shared_ptr<ChildContainerType> child_container =
      std::make_shared<ChildContainerType>(db, secdb, penv);
  std::shared_ptr<ParentContainerType> parent_container =
      std::make_shared<ParentContainerType>(parent_db, penv,
                                            ParentDeleterType(child_container));
...
void ChildStorageTest::testSeveralLevelsOfInheritance()
{
  ...
  using ChildThatIsParentDeleterType =
      ChildThatIsParentDeleter<KeyType, TestElement, TestElement,
                               ChildContainerType>;
  using ChildThatIsParentContainerType =
      ChildStorage<TestElement, TestElement, TestMarshaller, TestWatcher,
                   DefaultTransactionManager, ChildThatIsParentDeleterType>;
  using ParentDeleterType =
      ParentsDeleter<KeyType, TestElement, ChildThatIsParentContainerType>;
  using ParentContainerType =
      Storage<TestElement, TestMarshaller, TestWatcher,
              DefaultTransactionManager, ParentDeleterType>;
  ...
  ;
  auto res = child_child_db->open(
      nullptr, "ChildStorageTest_testStorageCreation.db", "child_child",
      DB_BTREE, DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);
  ...
  res = child_child_secdb->open(
      nullptr, "ChildStorageTest_testStorageCreation.db", "child_child_secdb",
      DB_BTREE, DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);

  QCOMPARE(0, res);

  child_child_db->associate(nullptr, child_child_secdb, get_dest_secdb_callback,
                            DB_CREATE);
  ...
  std::shared_ptr<ChildThatIsParentContainerType> child_that_is_parent =
      std::make_shared<ChildThatIsParentContainerType>(
          child_child_db, child_child_secdb, penv,
          ChildThatIsParentDeleterType(child_container));
  std::shared_ptr<ParentContainerType> parent_container =
      std::make_shared<ParentContainerType>(
          parent_db, penv, ParentDeleterType(child_that_is_parent));
