...
StoreWithWatcherTest::StoreWithWatcherTest()
{
  ...
  auto res =
      penv->open(".",
                 DB_INIT_LOCK | DB_INIT_LOG | DB_INIT_MPOOL | DB_INIT_TXN |
                     DB_CREATE | DB_PRIVATE | DB_RECOVER | DB_THREAD,
                 0600);
  ...
  res = db->open(nullptr, "StoreWithWatcherTest.db", "master", DB_BTREE,
  ...
  res = secdb->open(nullptr, "StoreWithWatcherTest.db", "secondary", DB_BTREE,
                    DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);
  ...
  res = parent_db->open(nullptr, "StoreWithWatcherTest.db", "parent", DB_BTREE,
                        DB_CREATE | DB_THREAD | DB_AUTO_COMMIT, 0600);
...
void StoreWithWatcherTest::testAddWatcher()
{
  ...
  using ParentContainerType =
      Storage<TestElement, TestMarshaller, EventQueueWatcher<TestElement>,
  ...
  parent_container->appendPermanentListener(EnqueuedEvents::ADDED,
                                            parentAddHandler);
  parent_container->appendPermanentListener(EnqueuedEvents::ALL_EVENTS,
                                            parentAddHandler);

  child_container->appendPermanentListener(EnqueuedEvents::ADDED,
                                           childAddHandler);
  child_container->appendPermanentListener(
      EnqueuedEvents::ADDED | EnqueuedEvents::UPDATED, childAddHandler);
...
void StoreWithWatcherTest::testRemoveParent()
{
  ...
  parent_container->appendPermanentListener(EnqueuedEvents::DELETED,
                                            parentRemoveHandler);
  auto parentHolder = parent_container->appendListener(
      EnqueuedEvents::UPDATED | EnqueuedEvents::DELETED, parentRemoveHandler);

  child_container->appendPermanentListener(EnqueuedEvents::DELETED,
                                           childRemoveHandler);
  auto childListener = child_container->appendListener(
      EnqueuedEvents::DELETED | EnqueuedEvents::UPDATED, childRemoveHandler);
