...
void ChangeWatcherTest::testAddEventHandling() {
  ...
  auto addedListener = [testEl, &calledCount](EnqueuedEvents event,
  ...
  watcher.appendPermanentListener(
      EnqueuedEvents::ADDED | EnqueuedEvents::UPDATED, addedListener);

  watcher.appendPermanentListener(EnqueuedEvents::ALL_EVENTS, addedListener);

  watcher.appendPermanentListener(
      EnqueuedEvents::DELETED | EnqueuedEvents::UPDATED, failedListener);
...
void ChangeWatcherTest::testAddEventTemporaryHandler() {
  ...
  {
    auto addHandler =
        watcher.appendListener(EnqueuedEvents::ADDED, addedListener);
    auto updateHandler =
        watcher.appendListener(EnqueuedEvents::UPDATED, failedListener);
    auto deleteHandler =
        watcher.appendListener(EnqueuedEvents::DELETED, failedListener);
    auto allEventsHandler =
        watcher.appendListener(EnqueuedEvents::ALL_EVENTS, addedListener);
    auto add_deleteHandlers = watcher.appendListener(
        EnqueuedEvents::ADDED | EnqueuedEvents::DELETED, addedListener);
    watcher.elementAdded(testEl);
...
void ChangeWatcherTest::testUpdateEventHandling() {
  ...
  auto updatedListener = [testEl, &calledCount](EnqueuedEvents event,
                                                const TestElement& element) {
  ...
  watcher.appendPermanentListener(
      EnqueuedEvents::ADDED | EnqueuedEvents::UPDATED, updatedListener);

  watcher.appendPermanentListener(EnqueuedEvents::ALL_EVENTS, updatedListener);

  watcher.appendPermanentListener(
      EnqueuedEvents::DELETED | EnqueuedEvents::ADDED, failedListener);
...
void ChangeWatcherTest::testUpdateEventTemporaryHandler() {
  ...
  {
    auto addHandler =
        watcher.appendListener(EnqueuedEvents::ADDED, failedListener);
    auto updateHandler =
        watcher.appendListener(EnqueuedEvents::UPDATED, updatedListener);
    auto deleteHandler =
        watcher.appendListener(EnqueuedEvents::DELETED, failedListener);
    auto allEventsHandler =
        watcher.appendListener(EnqueuedEvents::ALL_EVENTS, updatedListener);
    auto add_deleteHandlers = watcher.appendListener(
        EnqueuedEvents::UPDATED | EnqueuedEvents::DELETED, updatedListener);
    watcher.elementUpdated(testEl);
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
  }
  watcher.elementUpdated(testEl);
...
void ChangeWatcherTest::testDeleteEventHandling() {
  ...
  auto deletedListener = [testEl, &calledCount](EnqueuedEvents event,
                                                const TestElement& element) {
  ...
  watcher.appendPermanentListener(
      EnqueuedEvents::ADDED | EnqueuedEvents::UPDATED, failedListener);

  watcher.appendPermanentListener(EnqueuedEvents::ALL_EVENTS, deletedListener);

  watcher.appendPermanentListener(
      EnqueuedEvents::DELETED | EnqueuedEvents::ADDED, deletedListener);
...
void ChangeWatcherTest::testDeleteEventTemporaryHandler() {
...

  {
    ...
    auto updateHandler =
        watcher.appendListener(EnqueuedEvents::UPDATED, failedListener);
    auto deleteHandler =
        watcher.appendListener(EnqueuedEvents::DELETED, deletedListener);
    auto allEventsHandler =
        watcher.appendListener(EnqueuedEvents::ALL_EVENTS, deletedListener);
    auto add_deleteHandlers = watcher.appendListener(
        EnqueuedEvents::UPDATED | EnqueuedEvents::DELETED, deletedListener);
    watcher.elementRemoved(testEl);
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
  }
  watcher.elementRemoved(testEl);
...
void ChangeWatcherTest::testTemporaryAndPermanentListeners() {
  ...
  auto addListener = [&addListenerCalledCount](EnqueuedEvents event,
                                               const TestElement&) {
    QCOMPARE(event, EnqueuedEvents::ADDED);
  ...
  auto updateListener = [&updateListenerCalledCount](EnqueuedEvents event,
                                                     const TestElement&) {
    QCOMPARE(event, EnqueuedEvents::UPDATED);
  ...
  auto deleteListener = [&deleteListenerCalledCount](EnqueuedEvents event,
                                                     const TestElement&) {
    QCOMPARE(event, EnqueuedEvents::DELETED);
  ...
  auto delete_and_updateListener = [&delete_and_update_ListenerCalledCount](
                                       EnqueuedEvents event,
                                       const TestElement&) {
    QVERIFY(EnqueuedEvents::DELETED == event ||
            EnqueuedEvents::UPDATED == event);
    delete_and_update_ListenerCalledCount++;
  };

  auto update_and_add_ListenerCalledCount = 0;
  auto update_and_addListener = [&update_and_add_ListenerCalledCount](
                                    EnqueuedEvents event, const TestElement&) {
  ...
  auto allEvent_addListener = [&allEvent_ListenerCalledCount](
                                  EnqueuedEvents, const TestElement&) {
  ...
  watcher.appendPermanentListener(
      EnqueuedEvents::DELETED | EnqueuedEvents::UPDATED,
      delete_and_updateListener);
  watcher.appendPermanentListener(
      EnqueuedEvents::UPDATED | EnqueuedEvents::ADDED, update_and_addListener);
  watcher.appendPermanentListener(EnqueuedEvents::ALL_EVENTS,
                                  allEvent_addListener);
  ...
  {
    auto addedHandler =
        watcher.appendListener(EnqueuedEvents::ADDED, addListener);
    auto newAddedHandler = std::move(addedHandler);
    auto updatedHandler =
        watcher.appendListener(EnqueuedEvents::UPDATED, updateListener);
    auto deletedHandler =
        watcher.appendListener(EnqueuedEvents::DELETED, deleteListener);
    auto updated_and_deletedHandler = watcher.appendListener(
        EnqueuedEvents::DELETED | EnqueuedEvents::UPDATED,
        delete_and_updateListener);
    auto updated_and_addedHandler =
        watcher.appendListener(EnqueuedEvents::UPDATED | EnqueuedEvents::ADDED,
                               update_and_addListener);
    auto all_events_Handler = watcher.appendListener(EnqueuedEvents::ALL_EVENTS,
                                                     allEvent_addListener);
