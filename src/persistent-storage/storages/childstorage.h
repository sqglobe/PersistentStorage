...
template <
    ...
    typename Deleter =
        DefaultChildDeleter<decltype(get_id(std::declval<Element>())), Element, Parent, DefaultDeleter<decltype(get_id(std::declval<Element>())), Element>>>
class ChildStorage: public Storage<Element, Marshaller, Watcher, TxManager, Deleter>{
public:
    using ParentContainer = Storage<Element, Marshaller, Watcher, TxManager, Deleter>;
    using ParentElementId = decltype(get_id(std::declval<Parent>()));


public:
    ChildStorage(Db* db, Db *secondary, DbEnv *env, Deleter&& deleter = Deleter());
